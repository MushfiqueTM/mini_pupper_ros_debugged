...
This branch targets **ROS 2 Jazzy** on **Ubuntu 24.04 (Noble Numbat)**.

- **ROS 2 Humble** development should target the `ros2-humble` branch
- **ROS 2 Jazzy** development should target the `ros2` branch
...
For Pull Requests, please target the `ros2` branch for any ROS 2 Jazzy contributions,
and the `ros2-humble` branch for any ROS 2 Humble contributions.  

**For ROS 2 Jazzy contributions:**
```sh
git checkout ros2                # start with the ros2 branch
git pull origin ros2             # pull remote repo changes
git checkout -b your-feature-branch # create your feature branch
```

**For ROS 2 Humble contributions:**
```sh
git checkout ros2-humble         # start with the ros2-humble branch
git pull origin ros2-humble      # pull remote repo changes
git checkout -b your-feature-branch # create your feature branch
```

Then when you submit a Pull Request, please select the appropriate target branch.
...
If you are migrating code from Humble, please refer to [MIGRATION_JAZZY.md](MIGRATION_JAZZY.md) for detailed instructions.
...
Please ensure that any new contributions are compatible with **C++17** and **Python 3.12**.
...
Since this branch uses Gazebo Harmonic (not Gazebo Classic), please ensure:

1. Use `ros_gz_sim` instead of `gazebo_ros`
2. Use `gz_sim.launch.py` instead of `gazebo.launch.py`
3. Use `ros_gz_sim create` instead of `spawn_entity.py`
4. Test your changes in simulation before submitting PR
