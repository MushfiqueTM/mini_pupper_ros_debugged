...
class Movements:
    ...
    def __init__(
        ...
        # init state value
        self.SpeedInit = [0.0, 0.0, 0.0]  # x, y speed
        self.AttitudeInit = [0.0, 0.0, 0.0]  # roll pitch yaw rate
        self.TurnInit = [0.0, 0.0, 0.0]  # yaw rate
        ...
        # output
        self.SpeedOutput = [0.0, 0.0, 0.0]  # x, y speed
        self.AttitudeOutput = [0.0, 0.0, 0.0]  # roll pitch yaw rate
        self.TurnOutput = [0.0, 0.0, 0.0]  # yaw rate
...
class MovementScheme:
    ...
    def __init__(self, movements_lib):
        ...
        self.turn_pre = [0.0, 0.0, 0.0]
        self.turn_now = [0.0, 0.0, 0.0]

        self.record_index_number = []

        # criterion that make sure delta of the movement transition
        # are obtained at the begining of the transition and doesn't
        # change later
    ...
    def updateMovement(self, movement_type):
        ...
        if (movement_type != self.movement_now_name):
            self.ststus = 'Exit'
        elif (self.entry_down and self.entry_down1
              and self.entry_down2 and self.entry_down3):
            self.ststus = 'Movement'
        elif (self.exit_down and self.exit_down1
              and self.exit_down2 and self.exit_down3):
            self.ststus = 'Entry'
...
#                             ].getAttitudeOutput('Init')))
            ...
            attitude_ready = np.array(
                self.movements_now.getAttitudeOutput('Init'))
            turn_ready = np.array(
                self.movements_now.getTurnOutput('Init'))

            if self.getAccCommand:
                self.movements_now.DeltLegsM = (
                    location_ready - self.legs_location_pre)
                self.movements_now.DeltLegsM = (
                    self.movements_now.DeltLegsM.astype(np.float64))
                self.movements_now.DeltLegsM /= self.movements_now.transTic

                self.movements_now.DeltSpeedM = (
                    speed_ready - self.speed_pre)
                self.movements_now.DeltSpeedM = (
                    self.movements_now.DeltSpeedM.astype(np.float64))
                self.movements_now.DeltSpeedM /= self.movements_now.transTic

                self.movements_now.DeltAttitudeM = (
                    attitude_ready - self.attitude_pre)
                self.movements_now.DeltAttitudeM = (
                    self.movements_now.DeltAttitudeM.astype(np.float64))
                self.movements_now.DeltAttitudeM /= (
                    self.movements_now.transTic)

                self.movements_now.DeltTurnM = (
                    turn_ready - self.turn_pre)
                self.movements_now.DeltTurnM = (
                    self.movements_now.DeltTurnM.astype(np.float64))
        ...
        if self.ststus == 'Exit':
            if self.movements_pre.ExitToStand:
                ...
                # update the speed
                self.speed_now, self.exit_down1 = (
                    self.updateMovementSpeedGradient(
                        self.speed_pre, SpeedStanding))
                self.legs_location_pre = self.legs_location_now
                ...
                self.turn_now, self.exit_down3 = (
                    self.updateMovementAttitudeGradient(
                        self.turn_pre, 0))
                self.turn_pre = self.turn_now

                if (self.exit_down and self.exit_down1
                        and self.exit_down2 and self.exit_down3):
            ...
            self.tick += 1
            if (self.tick >= self.now_ticks
                    and self.movement_now_number
                    < len(self.movements_lib) - 1):
    ...
    def updateMovementAttitudeGradient(
        ...
        # ensure output elements are native Python floats
        attitude_gradient = [float(val) for val in attitude_gradient]
    ...
    def updateMovementTurnGradient(self, turn_now, turn_target):
        ...
        if abs(diff) > abs(self.movements_now.DeltTurnM[0]):
            turn_gradient[0] = (
                turn_now[0] + self.movements_now.DeltTurnM[0])
        else:
    ...
    def updateLegsScheme(self):
        ...
        self.legs_location_now = (
            self.movements_now.getLegsLocationOutput('normal'))

    def updateAttitudeScheme(self):
        self.movements_now.runAttitudeSequence()
        self.attitude_now = (
            self.movements_now.getAttitudeOutput('normal'))
