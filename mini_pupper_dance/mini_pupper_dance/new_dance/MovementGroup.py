...
#                 Level 3(for beyond): Samples delicately control
#                    the foot locations, move speed, and attitudes
...
class MovementGroups:
    ...
    def head_move(self, pitch_deg=0, yaw_deg=0, time_uni=1, time_acc=1):
        ...
        Args:
            ...
            yaw_deg: the angle you want the robot's head to look left or right
                        e.g. 20 ----> the pupper will look right 20 degrees
        ...
        modified_pitch = self.cap_limit(
            self.pitchcap, -self.pitchcap, pitch_deg)
        modified_yaw = self.cap_limit(
            self.yawcap, -self.yawcap, yaw_deg)
        ...
        dance_scheme.setAllSequence(
            dance_all_legs, dance_speed, dance_attitude)
        self.MovementLib.append(dance_scheme)
        return self.MovementLib
    ...
    def body_row(self, row_deg=0, time_uni=1, time_acc=1):
        ...
        dance_all_legs = self.default_stand
        dance_speed = [[0, 0, 0]]
        dance_attitude = [[modified_row, 0, 0], [modified_row, 0, 0]]
        dance_scheme.setInterpolationNumber(interval_uni)
        dance_scheme.setTransitionTic(interval_acc)
        dance_scheme.setAllSequence(
            dance_all_legs, dance_speed, dance_attitude)
    ...
    def gait_uni(self, v_x=0, v_y=0, time_uni=1, time_acc=1):
        ...
        Args:
            ...
            time_acc: how long it takes to accelerate (unit: second)
    ...
    def height_move(self, ht=0, time_uni=1, time_acc=1):
        ...
        Args:
            ...
            time_uni: how long pupper will hold the position (unit:second)
        Return:
    ...
    def bowback(self, angle):
        ...
        dance_scheme.setAttitudeSequence(
            [[0, -modified_angle, 0], [0, -modified_angle, 0]],
            "Forever")
        dance_scheme.setSpeedSequence(dance_speed)
    ...
    """These Samples delicately control the foot locations,
    move speed and attitudes at each execution times,
    enabling more complicated movement based on your demands.
    By defaut, no arguments are needed, but you can
    ...
    def body_cycle(self):
        ...
        dance_all_legs.append([
            [0.06 + np.cos(1 * deg) * Radius,
             -0.05 + np.sin(1 * deg) * Radius, -0.07],
            [0.06 + np.cos(2 * deg) * Radius,
             -0.05 + np.sin(2 * deg) * Radius, -0.07],
            [0.06 + np.cos(3 * deg) * Radius,
             -0.05 + np.sin(3 * deg) * Radius, -0.07],
            [0.06 + np.cos(4 * deg) * Radius,
             -0.05 + np.sin(4 * deg) * Radius, -0.07],
            [0.06 + np.cos(5 * deg) * Radius,
             -0.05 + np.sin(5 * deg) * Radius, -0.07],
            [0.06 + np.cos(6 * deg) * Radius,
             -0.05 + np.sin(6 * deg) * Radius, -0.07],
            [0.06 + np.cos(7 * deg) * Radius,
             -0.05 + np.sin(7 * deg) * Radius, -0.07],
            [0.06 + np.cos(8 * deg) * Radius,
             -0.05 + np.sin(8 * deg) * Radius, -0.07],
            [0.06 + np.cos(9 * deg) * Radius,
             -0.05 + np.sin(9 * deg) * Radius, -0.07],
            [0.06 + np.cos(10 * deg) * Radius,
             -0.05 + np.sin(10 * deg) * Radius, -0.07],
            [0.06 + np.cos(11 * deg) * Radius,
             -0.05 + np.sin(11 * deg) * Radius, -0.07],
            [0.06 + np.cos(12 * deg) * Radius,
             -0.05 + np.sin(12 * deg) * Radius, -0.07],
            [0.06 + np.cos(13 * deg) * Radius,
             -0.05 + np.sin(13 * deg) * Radius, -0.07],
            [0.06 + np.cos(14 * deg) * Radius,
             -0.05 + np.sin(14 * deg) * Radius, -0.07],
            [0.06 + np.cos(15 * deg) * Radius,
             -0.05 + np.sin(15 * deg) * Radius, -0.07],
            [0.06 + np.cos(16 * deg) * Radius,
             -0.05 + np.sin(16 * deg) * Radius, -0.07],
        ...
        dance_scheme.setAttitudeSequence(
            dance_attitude, "Multiple", 3)
        dance_scheme.setSpeedSequence(
            dance_speed, "Multiple", 3)
        self.MovementLib.append(dance_scheme)
    ...
    def head_ellipse(self):
        ...
        dance_attitude = [
            [0, 20 * 1.414 / 2, 30 * 1.414 / 2],
            [0, 20 * 0.38268, 30 * 0.92388],
            [0, 0, 30],
            [0, -20 * 0.38268, 30 * 0.92388],
            [0, -20 * 1.414 / 2, 30 * 1.414 / 2],
            [0, -20 * 0.92388, 30 * 0.38268],
            [0, -20, 0],
            [0, -20 * 0.92388, -30 * 0.38268],
            [0, -20 * 1.414 / 2, -30 * 1.414 / 2],
            [0, -20 * 0.38268, -30 * 0.92388],
